/*
	Rollup.js v0.25.1
	Thu Jan 21 2016 12:35:51 GMT-0500 (EST) - commit c750730d9ba2ba3395e28c1a47dc8c8c27019c67
...
pp$2.parseTopLevel = function (node) {
  var first = true;
...
pp$2.parseStatement = function (declaration, topLevel) {
...
pp$2.parseBreakContinueStatement = function (node, keyword) {
...
pp$2.parseDebuggerStatement = function (node) {
  this.next();
  this.semicolon();
  return this.finishNode(node, "DebuggerStatement");
};

pp$2.parseDoStatement = function (node) {
  this.next();
...
pp$2.parseForStatement = function (node) {
  this.next();
  this.labels.push(loopLabel);
...
pp$2.parseFunctionStatement = function (node) {
  this.next();
  return this.parseFunction(node, true);
};

pp$2.parseIfStatement = function (node) {
  this.next();
  node.test = this.parseParenExpression();
...
pp$2.parseReturnStatement = function (node) {
...
pp$2.parseSwitchStatement = function (node) {
  this.next();
...
pp$2.parseThrowStatement = function (node) {
  this.next();
...
pp$2.parseTryStatement = function (node) {
  this.next();
...
pp$2.parseVarStatement = function (node, kind) {
  this.next();
...
pp$2.parseWhileStatement = function (node) {
  this.next();
  node.test = this.parseParenExpression();
  this.labels.push(loopLabel);
  node.body = this.parseStatement(false);
...
pp$2.parseWithStatement = function (node) {
...
pp$2.parseEmptyStatement = function (node) {
  this.next();
  return this.finishNode(node, "EmptyStatement");
};

pp$2.parseLabeledStatement = function (node, maybeName, expr) {
  for (var i = 0; i < this.labels.length; ++i) {
...
pp$2.parseExpressionStatement = function (node, expr) {
...
pp$2.parseBlock = function (allowStrict) {
...
pp$2.parseFor = function (node, init) {
  node.init = init;
  this.expect(tt.semi);
...
pp$2.parseForIn = function (node, init) {
...
pp$2.parseVar = function (node, isFor, kind) {
  node.declarations = [];
...
pp$2.parseFunction = function (node, isStatement, allowExpressionBody) {
...
pp$2.parseFunctionParams = function (node) {
  this.expect(tt.parenL);
...
pp$2.parseClass = function (node, isStatement) {
  this.next();
...
pp$2.parseClassMethod = function (classBody, method, isGenerator) {
  method.value = this.parseMethod(isGenerator);
  classBody.body.push(this.finishNode(method, "MethodDefinition"));
};

pp$2.parseClassId = function (node, isStatement) {
  node.id = this.type === tt.name ? this.parseIdent() : isStatement ? this.unexpected() : null;
};

pp$2.parseClassSuper = function (node) {
  node.superClass = this.eat(tt._extends) ? this.parseExprSubscripts() : null;
};

// Parses module export declaration.

pp$2.parseExport = function (node) {
  this.next();
  // export * from '...'
  if (this.eat(tt.star)) {
...
pp$2.shouldParseExportStatement = function () {
  return this.type.keyword;
};

// Parses a comma-separated list of module exports.

pp$2.parseExportSpecifiers = function () {
  var nodes = [],
      first = true;
  // export { x, y as z } [from '...']
  this.expect(tt.braceL);
...
pp$2.parseImport = function (node) {
  this.next();
  // import '...'
...
pp$2.parseImportSpecifiers = function () {
  var nodes = [],
      first = true;
...
pp$4.checkPropClash = function (prop, propHash) {
...
pp$4.parseExpression = function (noIn, refDestructuringErrors) {
...
pp$4.parseMaybeAssign = function (noIn, refDestructuringErrors, afterLeftParse) {
...
pp$4.parseMaybeConditional = function (noIn, refDestructuringErrors) {
  var startPos = this.start,
      startLoc = this.startLoc;
...
pp$4.parseExprOps = function (noIn, refDestructuringErrors) {
  var startPos = this.start,
      startLoc = this.startLoc;
...
pp$4.parseExprOp = function (left, leftStartPos, leftStartLoc, minPrec, noIn) {
...
pp$4.parseMaybeUnary = function (refDestructuringErrors) {
  if (this.type.prefix) {
    var node = this.startNode(),
...
pp$4.parseExprSubscripts = function (refDestructuringErrors) {
  var startPos = this.start,
      startLoc = this.startLoc;
...
pp$4.parseSubscripts = function (base, startPos, startLoc, noCalls) {
  for (;;) {
    if (this.eat(tt.dot)) {
      var node = this.startNodeAt(startPos, startLoc);
      node.object = base;
...
pp$4.parseExprAtom = function (refDestructuringErrors) {
  var node = undefined,
...
pp$4.parseParenAndDistinguishExpression = function (canBeArrow) {
  var startPos = this.start,
      startLoc = this.startLoc,
      val = undefined;
  if (this.options.ecmaVersion >= 6) {
...
pp$4.parseParenArrowList = function (startPos, startLoc, exprList) {
...
pp$4.parseTemplateElement = function () {
...
pp$4.parsePropertyValue = function (prop, isPattern, isGenerator, startPos, startLoc, refDestructuringErrors) {
...
pp$4.parseMethod = function (isGenerator) {
  var node = this.startNode();
  this.initFunction(node);
  this.expect(tt.parenL);
...
pp$4.parseArrowExpression = function (node, params) {
  this.initFunction(node);
...
pp$4.parseFunctionBody = function (node, isArrowFunction) {
...
pp$4.checkParams = function (node) {
  var nameHash = {};
...
pp$4.parseIdent = function (liberal) {
  var node = this.startNode();
...
pp$4.parseComprehension = function (node, isGenerator) {
  node.blocks = [];
...
pp$3.raise = function (pos, message) {
...
var SyntheticNamespaceDeclaration = (function () {
	...
	SyntheticNamespaceDeclaration.prototype.addReference = function addReference(reference) {
		...
		if (!this.needsNamespaceBlock) {
			...
			keys(this.originals).forEach(function (name) {
				_this2.originals[name].addReference(new SyntheticReference(name));
			});
		}
	...
	SyntheticNamespaceDeclaration.prototype.renderBlock = function renderBlock(indentString) {
		...
		var members = keys(this.originals).map(function (name) {
			var original = _this3.originals[name];
...
function umd(bundle, magicString, _ref, options) {
	...
	if (exportMode === 'named') {
		...
		globalDeps.unshift('(' + setupNamespace$1(options.moduleName) + ' = {})');
...
var Bundle = (function () {
	...
	Bundle.prototype.deconflict = function deconflict() {
		...
		this.externalModules.forEach(function (module) {
			...
			keys(module.declarations).forEach(function (name) {
	...
	Bundle.prototype.fetchAllDependencies = function fetchAllDependencies(module) {
		...
		var promises = module.sources.map(function (source) {
