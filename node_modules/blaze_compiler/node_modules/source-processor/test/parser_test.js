...
var fs = require('fs');
var Json = require('../src/source-processor');

function testParseJson(test) {
    var json = Json.parse(fs.readFileSync("test/parser_test.json", { encoding: 'utf8' }).toString());
    ...
    //"num":1,
    var num = json.asObject().getOrThrow("num", "fail").cast(1 /* JNumber */);
    ...
    var bool = json.asObject().getOrThrow("bool", "fail").cast(0 /* JBoolean */);
    ...
    var float = json.asObject().getOrThrow("float", "fail").cast(1 /* JNumber */);
    ...
    var array = json.asObject().getOrThrow("array", "fail").cast(5 /* JArray */);
    ...
    var obj = json.asObject().getOrThrow("object", "fail").cast(6 /* JObject */);
...
function testParseYaml(test) {
    var json = Json.parse_yaml(fs.readFileSync("test/complicated.yaml", { encoding: 'utf8' }).toString());
...
function compareJSYamlWithCustomJSJson(test) {
    //reading the data using our custom js-yaml parser
    var jsyaml = Json.parse_yaml(fs.readFileSync("test/complicated.yaml", { encoding: 'utf8' }).toString());

    //reading the data using our custom json parser (after using OTS js-yaml at the command line)
    var json = Json.parse(fs.readFileSync("test/complicated.json", { encoding: 'utf8' }).toString());

    //the two should produce the same data!
    test.deepEqual(json.toJSON(), jsyaml.toJSON());
    test.done();
}
exports.compareJSYamlWithCustomJSJson = compareJSYamlWithCustomJSJson;
